...
on:
  workflow_dispatch:
  schedule:
    - cron: '30 0 * * 0' # 对应 UTC 周日 00:30（北京时间周日 08:30 运行）
    ...
    steps:
      ...
      - name: 🚀 运行续期脚本
        env:
          ...
          WX_PUSH_TOKEN: ${{ secrets.WX_PUSH_TOKEN }} # 注入微信 Token
          IS_PROXY: ${{ env.IS_PROXY }}
          PROXY_SERVER: ${{ env.PROXY_SERVER }}
        run: xvfb-run --auto-servernum --server-args="-screen 0 1920x1080x24" python3 app.py
      ...
      - name: 🧽 清理进程和临时文件
        ...
        run: |
          ...
          pkill -f chrome || true
          pkill -f Xvfb || true
          rm -f sing-box sing-box-config.json sing-box.log
